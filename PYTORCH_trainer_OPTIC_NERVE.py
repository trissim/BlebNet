...
if __name__ == '__main__':
    ...
    s_path = '.\\checkpoint\\'; 
    """ path to input data
    input images must end in '_input.tif'
    and truth images must en in '_trut'.tif'
    """
    input_path = ".\\training images\\ONC Confocal\\all\\"; 

    """ if you get an out of memory error, try reducing batch_size"""
    batch_size = 2;      
    """ratio of images to be used for training and validation
    0.1 means 90% is used for training and 10% for validation"""
    test_size = 0.1 

    """enable image transforms. Creates new training data from existing data
    by applying image transformations to the training data. Set to 0 to turn off"""
    ...
    if not onlyfiles_check:   ### if no old checkpoints found, start new network and tracker
        ...
        """Load dataset again"""
        tracker.dataset = input_path
        idx_train, idx_valid, empty, empty = train_test_split(counter, counter, test_size=test_size, random_state=2018)
        tracker.idx_train = idx_train
        tracker.idx_valid = idx_valid
    ...
    for cur_epoch in range(len(tracker.train_loss_per_epoch), 10000): 
         ...
         for batch_x, batch_y, spatial_weight in training_generator:
                ...
                """ Plot for debug """    
                # show input images that have been transformed
