...
"""process thresholded images from inference"""
def process_output(output_stack,input_im_stack,min_slices, all_results,filename,sav_dir):
    """ Pre-processing """
    # 1) get more data (and good data)
    # 2) overlay seg masks and binarize to get better segmentations???

    """ Post-processing """
    """ (1) removes all things that do not appear in > min_slices!!!"""
    if len(output_stack) > 0:
        all_seg, all_blebs, all_eliminated = slice_thresh(output_stack, slice_size=min_slices)
    else:
        all_results[str(min_slices)].append(0)
        return all_results
    ...
    if len(np.unique(input_im_stack)) > 1:
    ...
    all_seg_THRESH, all_blebs_THRESH, all_eliminated_THRESH = slice_thresh(blebs_opened_masked, slice_size=min_slices)
    ...
    total_blebs = 0
    with open(os.path.join(sav_dir,"bleb_count.txt"),"w") as fp:
        fp.write(str(len(cc_overlap)))
    all_results[str(min_slices)].append(len(cc_overlap))
    return all_results
